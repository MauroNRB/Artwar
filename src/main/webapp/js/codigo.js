...
function preload() {
    ...
    if (alt == 768) {
        ...
        game.load.spritesheet('soldado1', 'img/mito/768.png', 115.88, 101);
        game.load.spritesheet('soldado2', 'img/lula/768.png', 113.76, 107);
    ...
    if (alt == 900 || alt >= 900) {
        ...
        game.load.spritesheet('soldado1', 'img/mito/900.png', 135.3, 118);
        game.load.spritesheet('soldado2', 'img/lula/900.png', 134, 126);
    ...
    if(alt != 768 && alt != 900 && alt < 900){
        game.load.spritesheet('soldado1', 'img/mito/soldadoP1.png', 95.25, 83);
        game.load.spritesheet('soldado2', 'img/lula/soldadoP2.png', 93.6, 88);
	...
	if(!reniciou) {
  		game.load.audio('boden', ['snd/mito.mp3', 'snd/mito.ogg']);    
    }
}
...
function update() {
    ...
    if (alt == 768) {
        if (E2 && !pistP2 && !deadP2 && !armaP2 && !pausa) {
            player2.body.velocity.x = -182.985;
            player2.animations.play('andandoE2');
        }
        if (D2 && !pistP2 && !deadP2 && !armaP2 && !pausa) {
            player2.body.velocity.x = 182.985;
            player2.animations.play('andandoD2');
        }
        if (cursors.up.isDown && player2.body.touching.down && hitPlatform2 && !deadP2) {
    ...
    if (alt == 768) {
        if (D1 && !pistP1 && !armaP1 && !deadP2 && !pausa) {
            player1.body.velocity.x = 182.985;
            player1.animations.play('andandoD');
        }
        if (E1 && !pistP1 && !armaP1 && !deadP2 && !pausa) {
            player1.body.velocity.x = -182.985;
            player1.animations.play('andandoE');
        }
        if (game.input.keyboard.isDown(Phaser.Keyboard.W) && player1.body.touching.down && hitPlatform1 && !deadP2) {
    ...
    if (game.input.keyboard.isDown(Phaser.Keyboard.ONE) && !deadP2) {
        pistP1 = true;
        armaP1 = false;
    }
    if (game.input.keyboard.isDown(Phaser.Keyboard.TWO) && !deadP2) {
        armaP1 = true;
        pistP1 = false;
    }
    if (game.input.keyboard.isDown(Phaser.Keyboard.E) && !deadP2 && !pausa) {
        tiro1 = true;
    }
    ...
    music.volume = 1;
    if(game.input.keyboard.isDown(Phaser.Keyboard.ENTER) && deadP2) {
        total = 0;
        deadP2 = false;
        vidaP1 = 0;
        vidaP2 = 0;
